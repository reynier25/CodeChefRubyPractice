...
t.times do
    ...
    a = STDIN.gets.chomp.split.map(&:to_i).sort
    biggest = m - 1
    second_biggest = m - 2
    smallest, ans = 0, 0
    while a[biggest] != n && second_biggest >= smallest
        a[smallest] -= 1
        smallest += 1 if a[smallest] == 0
        ans += 1
        a[second_biggest] = a[biggest] + a[second_biggest] + 1
        second_biggest -= 1
        biggest -= 1
    end
    puts ans
