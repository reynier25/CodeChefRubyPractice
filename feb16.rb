...
def solve
  n, height = gets.split.map(&:to_i)
  enter = Array.new(n + 1, 0)
  ext = Array.new(n + 1, 0)
  work = Array.new(n + 1, 0)
  n.times do
  ...
  cur_cell_count = n
  n.times do |i|
    cur_cell_count -= enter[i]
    cur_cell_count += ext[i]
    work[i + 1] = work[i] + cur_cell_count
  end
  (n - height + 1).times.map { |i| work[i + height] - work[i]}.min
end
