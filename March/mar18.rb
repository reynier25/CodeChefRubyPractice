...
def check_if_double_string(string)
  odd_length = string.size % 2
  incremental_length = (string.size/2) + odd_length
  (string.size/2).times do |i|
    if(string[i] != string[i + incremental_length])
      puts "NO"
      return
    end
  end
...
count.to_i.times do
  string_list << gets.chomp
end

string_list.each do |group|
  check_if_double_string(group)
end
