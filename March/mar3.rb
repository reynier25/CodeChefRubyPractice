...
require 'set'
t = gets.to_i # testcases
t.times do 
	n = gets.to_i # number of lines
	map = Hash.new{ |map,k| map[k] = Set.new } # key=>slope,value=>set of lines
	n.times do
		points = gets.chomp.split(" ").map(&:to_f) # get A,B,C
		if points[1]!=0 # check if B is 0
			points[0]/=points[1]
			points[2]/=points[1]
			points[1]=1
			slope=-points[0]
		...
		map[slope] << points # add new points to set
	end
	# print map
	max = -1
	map.each do |k,v|
		if v.size > max #check number of lines for each slope and save max
			max = v.size
		end
	end
	puts max # print max
