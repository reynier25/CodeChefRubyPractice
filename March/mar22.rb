...
(0..test_cases-1).each do |j|
	input = gets.strip
	k = gets.chomp.to_i
	input_array = input.split
	arr=[]
	red = input_array[0].to_i
	green = input_array[1].to_i
	blue = input_array[2].to_i
	arr << red 
	arr << blue 
	arr << green 
	arr.sort!
	if k <= arr[0] and k <= arr[1] and k <= arr[2] 
		p (k-1)*3 + 1 
	elsif k >= arr[0] and k <= arr[1] and k <= arr[2] 
		p  arr[0]+ (k-1)*2 +1
	else
		p arr[0]+arr[1]+k
	end
