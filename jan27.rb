...
(1..test_cases).each do |test_case|
   types_of_ingredients = gets().strip.to_i
   quantity_of_ingredients = gets().strip.split(" ").compact.map(&:to_i)
   count = 0 
   min_quantity = quantity_of_ingredients.min
   if min_quantity < 2 
   	 puts -1 
   	 next 
   end
   quantity_of_ingredients.each {|quantity| count = count + quantity.to_i } 
   puts count - min_quantity + 2
end
