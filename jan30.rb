...
# Mountain View
# Where max(s) represents the maximum value in set s whereas min(s) represents the minimum value in the set s. As value of SETDIFF(S) can be very large, print it modulo (109 + 7) .
...
T.times do
    ...
    ans = 0
    (0..(n-2)).each do |i|
        #puts "#{power[n-2-i]} #{sum_r[i]} #{sum[i]}"
        ans = (ans%mod + (power[n-2-i]*((sum_r[i]%mod - sum[i]%mod)%mod)%mod)%mod)%mod
    end
    puts ans
end
