...
# Example case 2. This is the sequence of cells visited:
...
gets.chomp.to_i.times.each do
  hash, num, count = {}, gets.chomp.to_i, 0
  num.to_i.times.each { |i| gets.split(' ').each_with_index { |element, index| hash[element.to_i] = Coard.new(i, index) } }
  (1...num**2).each{ |i| count += hash[i+1].diff(hash[i]) }
  p count
end
