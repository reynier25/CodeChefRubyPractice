...
def power2? num
	# Convert num to binary string & check if string is all 1's
...
t.times do
	...
	steps = 0
	# Check if a is a number is 2^power
	unless power2? a
		# loop until a becomes 2^anything or 1
		until power2? a
			a = a/2
			steps += 1.0
		end
	...
	# puts "#{a} #{b}"
	if a > b
		steps += Math.log2(a) - Math.log2(b)
	elsif a < b
		steps += Math.log2(b) - Math.log2(a)		
	end
	puts steps.to_i
end
