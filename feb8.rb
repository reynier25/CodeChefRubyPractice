...
def largest_sum_diameters(first_row_dimensions, second_row_dimensions)
    first_row_dimensions.sort!
    second_row_dimensions.sort!
    
    sum = 0
    
    (0...first_row_dimensions.length).each do |idx|
        if first_row_dimensions[idx] <= second_row_dimensions[idx]
            sum += first_row_dimensions[idx]
        else
            sum += second_row_dimensions[idx]
        end
    end
...
num_test_cases = gets        # get input
num_test_cases ||= ''        # set to empty string if nil
num_test_cases.chomp!
num_test_cases = num_test_cases.to_i # if empty string, sets to 0 upon conversion to Integer
...
while i < num_test_cases
	...
	num_rectangles = gets.chomp().to_i
	
	first_row_dimensions = gets.chomp().split(" ").map(&:to_i)
	second_row_dimensions = gets.chomp().split(" ").map(&:to_i)
	
	if i + 1 != num_test_cases
	    puts largest_sum_diameters(first_row_dimensions, second_row_dimensions)
	else
	    print largest_sum_diameters(first_row_dimensions, second_row_dimensions)
	end
