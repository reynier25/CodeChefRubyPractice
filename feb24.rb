...
1.upto(t) do |tt|
  ...
  0.upto(n-1) do |j|
  	count += 1 if str1[j] == str2[j]
  end
  
  
  winnings_arr = gets.chomp.split(" ")
  max = 0
  ...
  0.upto(count) do |x|
  	max = winnings_arr[x].to_i if winnings_arr[x].to_i > max
  end
...
T.times do
  ...
  count = 0
  n.times do |index|
    count += 1 if a[index] == b[index]
    answer = [answer, c[count]].max
  end
  answer = c[n] if count == n
  puts answer
end
